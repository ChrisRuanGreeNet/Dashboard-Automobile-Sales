...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        ...
        y='Automobile_Sales',title='Average number of vehicles sold by vehicle type'))
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        unemploy_sales= recession_data.groupby("Vehicle_Type")['Automobile_Sales'].sum().reset_index()
        R_chart4 = dcc.Graph(figure=px.bar(unemploy_sales, x="Vehicle_Type", y='Automobile_Sales',title="effect of unemployment rate on vehicle type and sales"))
...
app.layout = html.Div([#TASK 2.1 Add title to the dashboard
    html.H1("Automobile Sales Statistics Dashboard", 
            style={'color':"#503D36", 'font_size':24}),
...
# Define the callback function to update the input container based on the selected statistics
@app.callback(
    Output(component_id='output-container', component_property='output-container'),
    [Input(component_id='select-year', component_property='select-year'), Input(component_id='dropdown-statistics', component_property='dropdown-statistics')])
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        # use groupby to create relevant data for plotting
        average_sales = recession_data['Automobile_Sales'].mean().reset_index()                           
        R_chart2  = dcc.Graph(figure=px.bar(average_sales, x='Vehicle_Type',
y='Automobile_Sales',title='Average number of vehicles sold by vehicle type  '))
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        unemploy_sales= recession_data.groupby("Vehicle_type")['Automobile_Sales'].sum().reset_index()
        R_chart4 = dcc.Graph(figure=px.bar(unemploy_sales, values="Vehicle_type",names='Automobile_Sales',title="effect of unemployment rate on vehicle type and sales"))
...
#plot 1 Yearly Automobile sales using line chart for the whole period.
        yas= data.groupby('Year')['Automobile_Sales'].mean().reset_index()
        Y_chart1 = dcc.Graph(figure=px.line(yas, x='Year', y='Automobile_Sales','Yearly Automobile sales using line chart for the whole period'))
            
# Plot 2 Total Monthly Automobile sales using line chart.
        mas = data.groupby('Month')['Automobile_Sales'].mean().reset_index()
        Y_chart2 = dcc.Graph(px.line( x='Month',y='Automobile_Sales','Total Monthly Automobile sales'))

            # Plot bar chart for average number of vehicles sold during the given year
        avr_vdata=yearly_data.groupby(input_year)['Automobile_Sales'].mean().reset_index
        Y_chart3 = dcc.Graph(px.bar(avr_vdata, x=input_year, y='Automobile_sales',title='Average Vehicles Sold by Vehicle Type in the year {}'.format(input_year)))

            # Total Advertisement Expenditure for each vehicle using pie chart
        exp_data=yearly_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum().reset_index()
        Y_chart4 = dcc.Graph(px.pie(exp_data, values='Vehicle_Type', y='Advertising_Expenditure', title = 'Total Advertisement Expenditure for each vehicle'))
